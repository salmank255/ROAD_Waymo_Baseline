...
from val import val
from torch.utils.data import ConcatDataset, Subset, SubsetRandomSampler, RandomSampler
...
def main():
    ...
    if args.MODE in ['train','val']:
        ...
        # train_t_dataset = Subset(train_t_dataset, range(len(train_s_dataset)))
        logger.info('Done Loading Target ({}) Train Dataset'.format(args.DATASET))
        # logger.info('number of classes: {}'.format(train_t_dataset.num_classes))
        logger.info(train_t_dataset.print_str)
        ...
        # # assume your two datasets are named 'dataset_1' and 'dataset_2'
        # num_samples_s = len(train_s_dataset)
        # num_samples_t = len(train_t_dataset)

        # # randomly sample #source dataset samples from the target dataset
        # indices_t = list(range(num_samples_t))
        # sampled_indices_t = RandomSampler(indices_t, num_samples=num_samples_s)
        # args.sampled_indices_t_train = sampled_indices_t

        # create dataloaders for both datasets using the sampled indices for the second dataset
        # batch_size = 32
        # dataloader_1 = torch.utils.data.DataLoader(dataset_1, batch_size=batch_size, shuffle=True)
        # dataloader_2 = torch.utils.data.DataLoader(dataset_2, batch_size=batch_size, sampler=sampled_indices_2)
        

        # print(train_s_dataset[0])
        # print(trains_t_dataset[0])
        # print(train_st_dataset[0])
    ...
    args.DATASET = 'road'
    args.SUBSETS = ['val_3']
    val_s_dataset = VideoDataset(args, train=False, transform=val_transform, skip_step=skip_step, full_test=full_test)
    logger.info('Done Loading Source ({}) Validation Dataset'.format(args.DATASET))
    
    args.num_samples = len(val_s_dataset)
    args.DATASET = 'roadpp'
    args.SUBSETS = ['val']
    val_t_dataset = VideoDataset(args, train=False, transform=val_transform, skip_step=skip_step, full_test=full_test)
    ...
    # logger.info('Train {} Dataset Size: {}'.format('Total', len(train_st_dataset)))
    # logger.info('Validation {} Dataset Size: {}'.format('road', len(val_s_dataset)))
    # logger.info('Validation {} Dataset Size: {}'.format('roadpp', len(val_t_dataset)))
    # logger.info('Validation {} Dataset Size: {}'.format('Total', len(val_st_dataset)))
    # logger.info(' w and h for {} are {} and {}'.format('road', train_s_dataset[0][5][0], train_s_dataset[0][5][1]))
    # logger.info(' w and h for {} are {} and {}'.format('roadpp', train_t_dataset.wh[0], train_t_dataset.wh[0]))

    args.num_classes =  val_t_dataset.num_classes
    # one for objectness
    args.label_types = val_t_dataset.label_types
    args.num_label_types = val_t_dataset.num_label_type
    args.all_classes =  val_t_dataset.all_classes
    args.num_classes_list = val_t_dataset.num_classes_list
    args.num_ego_classes = val_t_dataset.num_ego_classes
    args.ego_classes = val_t_dataset.ego_classes
    args.head_size = 256
    ...
    #     build_eval_tubes(args, val_s_dataset)
